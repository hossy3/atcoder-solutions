...
fn main() {
    ...
    for i in 0..n {
        let mut v = vec![hwd[i].0, hwd[i].1, hwd[i].2];
        v.sort();
        hwd[i] = (v[0], v[1], v[2]);
    }

    hwd.sort_by(|(h0, w0, d0), (h1, w1, d1)| (h0, Reverse(w0), d0).cmp(&(h1, Reverse(w1), d1)));
    ...
    for i in 0..3 {
        ...
        for j in 0..n {
            match i {
                0 => hwd[j].0 = *map.get(&hwd[j].0).unwrap(),
                1 => hwd[j].1 = *map.get(&hwd[j].1).unwrap(),
                _ => hwd[j].2 = *map.get(&hwd[j].2).unwrap(),
            };
        }
    }

    let segtree = vec![std::usize::MAX; n + 1];
    let mut segtree: Segtree<Min<_>> = segtree.into();
