...
fn f(n: usize, s: &mut [char], klr: &[(usize, usize, usize)]) -> Vec<bool> {
    let mut results = vec![];
    let mut segtree: LazySegtree<F> = vec![0; n + 2].into();
    for (i, &c) in s.iter().enumerate() {
        let diff = if c == '(' { 1 } else { -1 };
        segtree.apply_range(i + 1, n + 2, diff);
    }
    for &(k, l, r) in klr {
        if k == 1 {
            if s[l - 1] == '(' && s[r - 1] == ')' {
                segtree.apply_range(l, r, -2);
                s.swap(l - 1, r - 1);
            } else if s[l - 1] == ')' && s[r - 1] == '(' {
                segtree.apply_range(l, r, 2);
                s.swap(l - 1, r - 1);
            ...
            let yes = segtree.get(l - 1) == segtree.get(r)
                && segtree.get(l - 1) == segtree.prod(l, r + 1);
            results.push(yes);
        }
    }
    results
...
fn main() {
    input! {
        ...
        q: usize,
        mut s: Chars,
        klr: [(usize, usize, usize); q],
    }
    let results = f(n, &mut s, &klr);
    for &yes in &results {
