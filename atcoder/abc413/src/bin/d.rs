...
fn f(a: &[i64]) -> bool {
    ...
    if a[0] == -a[n - 1] {
        if n % 2 == 0 {
            if a[n / 2 - 1] == a[0] && a[n / 2] == a[n - 1] {
                return true;
            }
        } else {
            if (a[n / 2 - 1] == a[0] && a[n / 2] == a[n - 1])
                || (a[n / 2] == a[0] && a[n / 2 + 1] == a[n - 1])
            {
                return true;
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_name() {
        assert_eq!(f(&[2, 2, 2]), true);
        assert_eq!(f(&[-2, 2, 2]), true);
        assert_eq!(f(&[-2, -2, 2]), true);
        assert_eq!(f(&[-2, -2, -2]), true);

        assert_eq!(f(&[2, 2, 2, 2]), true);
        assert_eq!(f(&[-2, 2, 2, 2]), false);
        assert_eq!(f(&[-2, -2, 2, 2]), true);
        assert_eq!(f(&[-2, -2, -2, 2]), false);
        assert_eq!(f(&[-2, -2, -2, -2]), true);

        assert_eq!(f(&[2, 2, 2, 2, 2]), true);
        assert_eq!(f(&[-2, 2, 2, 2, 2]), false);
        assert_eq!(f(&[-2, -2, 2, 2, 2]), true);
        assert_eq!(f(&[-2, -2, -2, 2, 2]), true);
        assert_eq!(f(&[-2, -2, -2, -2, 2]), false);
        assert_eq!(f(&[-2, -2, -2, -2, -2]), true);
    ...
    fn test2() {
        ...
        assert_eq!(
            f(&[90000, 8100, -27000, 729, -300000, -2430, 1000000]),
            true
        );
