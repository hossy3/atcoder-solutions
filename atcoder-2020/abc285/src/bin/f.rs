...
fn main() {
    ...
    let mut segs = Vec::with_capacity(26);
    for _ in 0..26 {
        segs.push(Segtree::<Additive<_>>::new(n));
    }
    let mut set = BTreeSet::new(); // reversed
    let mut v = Vec::with_capacity(n);

    let mut counts = vec![0; 26];
    for (i, &c) in s.iter().enumerate() {
        let j = c as usize - b'a' as usize;
        segs[j].set(i, 1);
        v.push(j);
        counts[j] += 1;
    }
    ...
    'outer: for _ in 0..q {
        ...
        if t == 1 {
            ...
            if let Some(_) = set.range((Included(l), Excluded(r))).next() {
                println!("No");
                continue 'outer;
            }

            let mut subcounts = Vec::with_capacity(26);
            for i in 0..26 {
                subcounts.push(segs[i].prod(l, r + 1));
            }
            if subcounts[v[l]] > counts[v[l]] || subcounts[v[r]] > counts[v[r]] {
                println!("No");
                continue 'outer;
            }
            if v[l] != v[r] {
                for i in (v[l] + 1)..v[r] {
                    if subcounts[i] != counts[i] {
                        println!("No");
                        continue 'outer;
                    }
                }
